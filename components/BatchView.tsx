      ...
      <div className="flex-1 flex flex-col min-w-0 border-r border-slate-800">
        <div className="p-6 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-bold text-slate-200">处理队列 ({images.length})</h3>
          </div>
        </div>
          ...
          {images.length === 0 ? (
            ...
            images.map(img => (
              <div key={img.id} className="flex items-center gap-4 p-3 bg-slate-800/40 rounded-xl border border-slate-700/30 group">
                ...
                {!processing && (
                  <button onClick={() => onRemove(img.id)} className="p-2 text-slate-600 hover:text-red-400 transition-all opacity-0 group-hover:opacity-100">
      ...
      <div className="w-full md:w-96 bg-[#0a0f1d] p-8 shrink-0 flex flex-col">
        <div className="mb-8 flex-1 overflow-y-auto pr-2 no-scrollbar space-y-8">
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-2">
                {['JPEG', 'PNG', 'WEBP'].map(fmt => (
                  <button 
                    ...
                    className={`py-2 rounded-lg text-xs font-bold border ${outputFormat.includes(fmt.toLowerCase()) ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}
                  >
                    {fmt}
                  </button>
                ))}
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2 mb-4">
                <button 
                  ...
                  className={`flex-1 py-2 text-xs font-bold rounded-lg border ${resizeMode === 'original' ? 'bg-indigo-600 border-indigo-500' : 'bg-slate-800 border-slate-700 text-slate-400'}`}
                >
                  保持原样
                </button>
                <button 
                  onClick={() => setResizeMode('custom')}
                  className={`flex-1 py-2 text-xs font-bold rounded-lg border ${resizeMode === 'custom' ? 'bg-indigo-600 border-indigo-500' : 'bg-slate-800 border-slate-700 text-slate-400'}`}
                >
                  自定义尺寸
                </button>
              </div>
              ...
              {resizeMode === 'custom' && (
                <div className="p-4 bg-slate-800/40 rounded-xl border border-slate-700 space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <button 
                      ...
                      className={`mt-4 p-1.5 rounded-md ${aspectLocked ? 'text-indigo-400 bg-indigo-400/10' : 'text-slate-600 bg-slate-700/50'}`}
                    >
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">智能增强</label>
              <button 
                onClick={() => setAutoBalance(!autoBalance)}
                className={`w-full flex items-center justify-between p-3 rounded-xl border ${autoBalance ? 'bg-indigo-600/10 border-indigo-500/50 text-indigo-400' : 'bg-slate-800 border-slate-700 text-slate-400'}`}
              >
                <span className="text-xs font-bold">自动场景优化</span>
                <div className={`w-4 h-4 rounded-md border ${autoBalance ? 'bg-indigo-500 border-indigo-400' : 'bg-slate-900 border-slate-700'}`}>
                   {autoBalance && <CheckCircle2 className="w-3 h-3 text-white m-auto" />}
                </div>
              </button>
            </div>
          </div>
          ...
          {processing ? (
            <div className="space-y-4">
              <div className="flex justify-between items-end mb-2">
                <span className="text-xs font-bold text-slate-200 flex items-center gap-2"><Loader2 className="w-3.5 h-3.5 animate-spin" /> 处理中...</span>
            ...
            <button onClick={startBatch} disabled={images.length === 0} className={`w-full py-4 rounded-2xl flex items-center justify-center gap-3 font-bold transition-all ${images.length > 0 ? 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-xl' : 'bg-slate-800 text-slate-500 cursor-not-allowed'}`}>
