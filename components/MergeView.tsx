...
const MergeView: React.FC<MergeViewProps> = ({ images, onUpload }) => {
  ...
  }, [selectedIds, images, direction, gap, padding, bgColor, align, texts]);
      ...
      <div className="flex-1 flex flex-col min-w-0 border-r border-slate-800 overflow-hidden">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-bold text-slate-200 text-sm">选择合并素材 (已选 {selectedIds.length}/4)</h3>
          </div>
          <label className="cursor-pointer bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="grid grid-cols-4 gap-3">
            {images.map(img => (
              <div key={img.id} onClick={() => toggleImage(img.id)} className={`relative aspect-square rounded-xl overflow-hidden cursor-pointer border-2 transition-all ${selectedIds.includes(img.id) ? 'border-indigo-500 scale-95 ring-4 ring-indigo-500/20' : 'border-slate-700/50 hover:border-slate-500'}`}>
        ...
        <div className="flex-[2] bg-slate-900/50 border-t border-slate-800 p-8 flex flex-col items-center justify-center overflow-hidden">
          {selectedIds.length > 0 ? (
            <div className="relative w-full h-full flex items-center justify-center">
               <canvas ref={canvasRef} className="max-w-full max-h-full shadow-2xl rounded-sm object-contain" />
            </div>
          ) : (
            <div className="text-center opacity-30"><Maximize className="w-12 h-12 mx-auto mb-3" /><p className="text-sm font-medium">请选择图片进行预览</p></div>
          )}
        </div>
      </div>

      <div className="w-full md:w-80 bg-[#0a0f1d] shrink-0 flex flex-col overflow-hidden">
        <div className="flex border-b border-slate-800 shrink-0">
          <button onClick={() => setActiveTab('layout')} className={`flex-1 py-4 flex flex-col items-center gap-1 transition-all border-b-2 ${activeTab === 'layout' ? 'bg-indigo-600/5 text-indigo-400 border-indigo-500' : 'text-slate-500 border-transparent'}`}><Settings2 className="w-4 h-4" /><span className="text-[9px] font-bold uppercase">布局</span></button>
          <button onClick={() => setActiveTab('text')} className={`flex-1 py-4 flex flex-col items-center gap-1 transition-all border-b-2 ${activeTab === 'text' ? 'bg-indigo-600/5 text-indigo-400 border-indigo-500' : 'text-slate-500 border-transparent'}`}><TypeIcon className="w-4 h-4" /><span className="text-[9px] font-bold uppercase">文字</span></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => setDirection('horizontal')} className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-xs font-bold transition-all ${direction === 'horizontal' ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}><ArrowRight className="w-4 h-4" /> 左右</button>
                  <button onClick={() => setDirection('vertical')} className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-xs font-bold transition-all ${direction === 'vertical' ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}><ArrowDown className="w-4 h-4" /> 上下</button>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <input type="color" value={bgColor} onChange={(e) => setBgColor(e.target.value)} className="w-full h-10 rounded-lg bg-transparent border border-slate-700 cursor-pointer p-0" />
              </div>
            </div>
          )}
          ...
          {activeTab === 'text' && (
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                {texts.map(t => (
                  <div key={t.id} onClick={() => setActiveTextId(t.id)} className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer ${activeTextId === t.id ? 'bg-indigo-600/10 border-indigo-500' : 'bg-slate-800/40 border-slate-700'}`}>
                    <span className="text-xs text-slate-200 truncate">{t.text}</span>
                    <button onClick={(e) => { e.stopPropagation(); removeText(t.id); }} className="text-slate-500 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                  </div>
                ))}
              </div>
              {activeText && (
                <div className="pt-4 border-t border-slate-800 space-y-6">
                  <input type="text" value={activeText.text} onChange={(e) => updateText(activeText.id, { text: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-xs text-white" />
                  ...
                  <input type="color" value={activeText.color} onChange={(e) => updateText(activeText.id, { color: e.target.value })} className="w-full h-8 bg-transparent border border-slate-700 cursor-pointer" />
                </div>
              )}
            </div>
          )}
        ...
        <div className="p-6 border-t border-slate-800 shrink-0">
          <button onClick={() => { const canvas = canvasRef.current; if (canvas) { const link = document.createElement('a'); link.download = 'merged.png'; link.href = canvas.toDataURL('image/png'); link.click(); } }} disabled={selectedIds.length < 2} className={`w-full py-4 rounded-2xl flex items-center justify-center gap-3 font-bold transition-all shadow-xl ${selectedIds.length >= 2 ? 'bg-indigo-600 hover:bg-indigo-500 text-white' : 'bg-slate-800 text-slate-500 cursor-not-allowed'}`}><Save className="w-5 h-5" /> 导出合并结果</button>
        </div>
      </div>
    </div>
  );
