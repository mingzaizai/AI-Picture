...
const EditorView: React.FC<EditorViewProps> = ({ image, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'adjust' | 'presets' | 'text' | 'magic' | 'color'>('adjust');
    ...
    <div className="h-full flex flex-col md:flex-row bg-[#0a0f1d]">
      <div className="flex-1 relative bg-slate-900 overflow-hidden flex items-center justify-center p-8">
        ...
        <div className="absolute top-6 left-1/2 -translate-x-1/2 flex gap-1.5 bg-slate-900/80 backdrop-blur-md px-4 py-2 rounded-full border border-slate-700/50 shadow-lg z-20">
          ...
          <button onClick={() => { saveToHistory(); setFilters(DEFAULT_FILTERS); setTransform(DEFAULT_TRANSFORM); setTexts([]); }} className="flex items-center gap-2 px-3 py-1 text-slate-400 hover:text-white hover:bg-slate-700/50 rounded-md text-xs font-bold transition-all"><RefreshCcw className="w-4 h-4" /> 重置</button>
        </div>
      ...
      <div className="w-full md:w-80 border-l border-slate-800 bg-[#0f172a] flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {activeTab === 'adjust' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <button onClick={() => setIsCropMode(!isCropMode)} className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${isCropMode ? 'bg-indigo-600 border-indigo-400' : 'bg-slate-800 border-slate-700'}`}><Scissors className="w-5 h-5 mb-1" /><span className="text-[9px] font-bold">裁剪</span></button>
              </div>
            </div>
          )}
              ...
              {texts.length > 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {texts.map(t => (
                      <div key={t.id} onClick={() => setActiveTextId(t.id)} className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${activeTextId === t.id ? 'bg-indigo-600/10 border-indigo-500' : 'bg-slate-800/40 border-slate-700 hover:border-slate-500'}`}>
                        <span className="text-xs text-slate-200 truncate pr-4">{t.text}</span>
                        <button onClick={(e) => { e.stopPropagation(); removeText(t.id); }} className="text-slate-500 hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeText && (
                <div className="space-y-6 pt-4 border-t border-slate-800">
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="color" value={activeText.color} onChange={(e) => updateText(activeText.id, { color: e.target.value })} className="w-full h-8 rounded-lg bg-transparent border border-slate-700 cursor-pointer p-0" />
                  </div>
                </div>
              )}
            </div>
          ...
          {activeTab === 'presets' && (
            <div className="grid grid-cols-2 gap-3">
              {FILTER_PRESETS.map((preset) => (
                <button key={preset.id} onClick={() => { saveToHistory(); setFilters({ ...preset.values }); }} className="group p-2 rounded-xl bg-slate-800/40 border border-slate-700/50 hover:border-indigo-500/50 transition-all">
                  <div className="aspect-video rounded-lg bg-slate-900 overflow-hidden mb-2">
                     <img src={image.preview} className="w-full h-full object-cover" style={{ filter: getFilterString(preset.values) }} />
                  </div>
                  <span className="text-[10px] font-bold text-slate-300 uppercase">{preset.name}</span>
                </button>
              ))}
            </div>
          )}
          ...
          {activeTab === 'color' && (
            <div className="p-6 bg-slate-800/50 rounded-2xl flex flex-col items-center gap-4 border border-slate-700/50 text-center">
              <div className="w-20 h-20 rounded-2xl shadow-inner border border-white/10" style={{ backgroundColor: pickedColor || 'transparent' }} />
              <button onClick={() => { if (!pickedColor) return; navigator.clipboard.writeText(pickedColor); setCopyFeedback(true); setTimeout(() => setCopyFeedback(false), 2000); }} disabled={!pickedColor} className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 rounded-xl text-xs font-bold flex items-center justify-center gap-2">
                {copyFeedback ? <CheckCircle2 className="w-4 h-4" /> : <Copy className="w-4 h-4" />} {pickedColor || '点击画面取色'}
              </button>
            </div>
          )}
        </div>
